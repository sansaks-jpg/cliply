...
def _sanitize_highlights(raw_highlights: object, duration: float) -> List[Dict]:
    ...
    for item in raw_highlights:
        ...
        start_val = item.get("start_time") if item.get("start_time") is not None else item.get("start")
        end_val = item.get("end_time") if item.get("end_time") is not None else item.get("end")
...
def call_highlight_api(
    ...
    for attempt in range(1, MAX_HIGHLIGHT_API_ATTEMPTS + 1):
        ...
        except (json.JSONDecodeError, ValueError, AttributeError, RuntimeError, requests.RequestException) as e:
            last_error = str(e)
