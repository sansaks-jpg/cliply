...
def call_openai_llm(prompt: str) -> str:
    ...
    logger.info(
        f"[LLM] Memanggil OpenAI API — Model: {OPENAI_MODEL} | Base URL: {OPENAI_BASE_URL} | Key Present: {bool(OPENAI_API_KEY)}"
    )
...
def call_anthropic_llm(prompt: str) -> str:
    ...
    logger.info(
        f"[LLM] Memanggil Anthropic API — Model: {ANTHROPIC_MODEL} | Key Present: {bool(ANTHROPIC_API_KEY)}"
    )
...
def call_gemini_llm(prompt: str) -> str:
    ...
    logger.info(
        f"[LLM] Memanggil Gemini API — Model: {GEMINI_MODEL} | Key Present: {bool(GEMINI_API_KEY)}"
    )
